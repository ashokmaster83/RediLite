...
Or compile manually:
```bash
g++ -std=c++17 -pthread -Iinclude src/*.cpp -o main
```
...
```bash
./main            # listens on 6379
./main 6380       # listens on 6380
```
...
Refer to [Concepts.md](Concepts.md)&[Commands&operations.md](Commands&operations.md) for a detailed description of the underlying concepts (TCP sockets, RESP, data structures, etc.) and real‑world usage scenarios for each command.

---

## Testing

You can verify functionality interactively or via scripts. See the test examples in `Commands&operations.md` or use the provided `test_all.sh` script for end‑to‑end validation.
